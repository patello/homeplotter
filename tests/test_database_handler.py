...
def test_database_handler__get_db_stats(db_handler):
    db_handler.connect()
    stats = db_handler.get_db_stats(["Transactions", "Processed", "Unprocessed", "Categories", "Text Matches", "ID Matches", "Tables"])
    assert stats["Transactions"] == 0
    assert stats["Unprocessed"] == 0
    assert stats["Processed"] == 0
    assert stats["Categories"] == 0
    assert stats["Text Matches"] == 0
    assert stats["ID Matches"] == 0
...
def test_database_handler__reset_table(db_handler):
    ...
    cursor.execute("INSERT INTO transactions(date, account_id,amount,adjusted_amount,transaction_text) VALUES(?,?,?,?,?);",(1,2,3,4,5))
    ...
    db_handler.reset_table("accounts")
